...
class Alist2Strm:
    ...
    async def run(self) -> None:
        ...
        def filter(path: AlistPath) -> bool:
            ...
            try:
                ...
                logger.warning(f"获取 {path._path} 本地路径失败：{e}")
                return False
            ...
            if not self.overwrite and local_path.exists():
                if path.suffix in self.download_exts:
                    ...
                    if local_path_stat.st_mtime < path.modified_timestamp:
                        logger.debug(
                            f"文件 {local_path.name} 已过期，需要重新处理 {path._path}"
                        )
                        return True
                    if local_path_stat.st_size < path.size:
                        logger.debug(
                            f"文件 {local_path.name} 大小不一致，可能是本地文件损坏，需要重新处理 {path._path}"
                        )
                        return True
                logger.debug(f"文件 {local_path.name} 已存在，跳过处理 {path._path}")
                return False
    ...
    def __get_local_path(self, path: AlistPath) -> Path:
        ...
        if self.flatten_mode:
            ...
            relative_path = path._path.replace(self.source_dir, "", 1)
