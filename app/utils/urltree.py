...
def structure_to_dict(text:str) -> dict:
    ...
    def parse_lines(start_index:int=0, indent_level:int=0) -> tuple[dict, int]:
        result_dict = {}
        i = start_index
        while i < len(lines):
            line = lines[i]
            ...
            if current_indent > indent_level:
                ...
                if len(parts) == 4:
                    key, value1, value2 = parts[0].strip(), parts[1].strip(), ":".join(parts[2:]).strip()
...
def dict_to_structure(dictionary:dict) -> str:
    """
    将字典转换为能够被 Alist 地址树识别的文本
    """
    def parse_dict(sub_dictionary: dict[str, str | list[str] | dict], indent: int = 0):
        result_str = ""
